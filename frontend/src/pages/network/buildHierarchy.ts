...
// =====================================================
// Helper: Format edges with visual properties (width, color, arrows).
// Exported so other modules can apply consistent styling without running layout.
// =====================================================
export function formatEdgesForGraph(
    nodes: NetworkNode[],
    edges: NetworkEdge[]
): NetworkEdge[] {
    // Compute degrees across the provided graph
    const degrees = computeNodeDegrees(nodes, edges);

    // Precompute descendant counts (child edges only, unique, cycles-safe)
    const childEdges = edges.filter(e => e.relationship_type === 'child');
    const childrenByParent = new Map<number, number[]>();
    for (const e of childEdges) {
        if (!childrenByParent.has(e.from)) childrenByParent.set(e.from, []);
        childrenByParent.get(e.from)!.push(e.to);
    }
    const nodeById = new Map<number, NetworkNode>(nodes.map(n => [n.id, n] as [number, NetworkNode]));
...
export async function buildHierarchy(
    ...
    const formattedEdges = formatEdgesForGraph(networkData.nodes, networkData.edges);
