...
impl RoutineProcessor {
    ...
    pub async fn process_routines(&self) -> Result<(), RoutineError> {
        ...
        while let Some(row) = result.next().await? {
            ...
            if let Err(e) = self.catch_up_routine(&routine).await {
                error!(
    ...
    pub async fn catch_up_routine(&self, routine: &Goal) -> Result<(), RoutineError> {
        ...
        while current_next_timestamp < current_timestamp {
